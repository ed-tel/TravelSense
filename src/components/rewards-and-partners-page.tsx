...
const Checkbox = React.forwardRef<
  HTMLButtonElement,
  React.ComponentProps<typeof BaseCheckbox> & { indeterminate?: boolean }
>(({ indeterminate, ...props }, ref) => {
  ...
  return <BaseCheckbox ref={ref ?? innerRef} {...props} />;
});
...
// Cleaned partner seed data (logos removed)
// Large static list of 25 partners with connected rewards
const initialPartners: Partner[] = [
  {
    id: 1,
...
const CATEGORY_STYLES: Record<string, { icon: React.ReactNode; color: string }> = {
  ...
  Location: {
    icon: <MapPin className="h-4 w-4 text-yellow-600" />,
...
addActivityLog?: (entry: { action: string; partner: string; dataType: string; status: 'success' | 'warning' | 'info' }) => void
}

export function RewardsAndPartnersPage({
...
initialTab = 'upload',
addActivityLog,
}: RewardsAndPartnersPageProps = {}) {
const [activeTab, setActiveTab] = useState(initialTab)

// Safely resolve reward icon functions even when objects were de-serialized (functions lost)
const getRewardIcon = (
  maybeIcon: any,
  type: 'travel' | 'dining' | 'shopping'
): React.ComponentType<{ className?: string }> => {
  if (typeof maybeIcon === 'function') {
    return maybeIcon as React.ComponentType<{ className?: string }>
  }
  switch (type) {
    case 'travel':
      return Plane
...
const saved = typeof window !== 'undefined' ? localStorage.getItem('travelsense_partners') : null
...
const saved = typeof window !== 'undefined' ? localStorage.getItem('travelsense_activeCategories') : null
if (saved) {
...
// Uploaded datasets state (Firestore-backed; localStorage is only cache/fallback)
const [uploadedDatasets, setUploadedDatasets] = useState<UploadedDataset[]>(() => {
const saved = typeof window !== 'undefined' ? localStorage.getItem('travelsense_uploadedDatasets') : null
...
const saved = typeof window !== 'undefined' ? localStorage.getItem('travelsense_redeemedRewards') : null
...
const saved = typeof window !== 'undefined' ? localStorage.getItem('travelsense_voucherCodes') : null
if (saved) {
...
// Persist uploaded datasets cache to localStorage (optional cache)
useEffect(() => {
...
// Firestore subscription for per-user datasets
useEffect(() => {
if (!user) {
setUploadedDatasets([])
...
const datasetsCol = collection(db, 'users', user.uid, 'datasets')
const q = query(datasetsCol, orderBy('createdAt', 'desc'))

const unsub = onSnapshot(
  q,
  (snap) => {
    const items: UploadedDataset[] = snap.docs.map((d) => {
      ...
      return {
        ...
        recordsCount: typeof data.recordsCount === 'number' ? data.recordsCount : 0,
        fileType: data.fileType || 'txt',
        storagePath: data.storagePath,
        downloadURL: data.downloadURL,
      }
    })
    setUploadedDatasets(items)
    setIsLoadingDatasets(false)
  },
  () => {
    setIsLoadingDatasets(false)
...
const allCategoriesToggled = partner.dataTypesRequired.every((dataType) => activeCategories.has(dataType))
if (!allCategoriesToggled) {
return { met: false, reason: 'categories' as const }
}

const allCategoriesHaveData = partner.dataTypesRequired.every((dataType) => {
  return uploadedDatasets.some((dataset) => dataset.category === dataType && dataset.status === 'success')
})

if (!allCategoriesHaveData) {
  return { met: false, reason: 'datasets' as const }
}

return { met: true, reason: null as any }
}
...
partner.reward.title.toLowerCase().includes(searchQuery.toLowerCase())
  ...
  const hasActiveCategory = partner.dataTypesRequired.some((dataType) => activeCategories.has(dataType))
...
!partnerSearchFilter || reward.partnerName.toLowerCase().includes(partnerSearchFilter.toLowerCase())
...
if (success) {
  setPartners((prev) =>
    prev.map((p) => (p.id === verifyingPartner.id ? { ...p, status: 'Active' as const, verificationStatus: 'verified' as const } : p))
  )
...
const selectedCategories = uploadedDatasets.filter((d) => selectedDatasets.includes(d.id)).map((d) => d.category)
return selectedPartner.dataTypesRequired.every((requiredType) => selectedCategories.includes(requiredType))
}

const handleReject = (partner: Partner) => {
setPartners((prev) => prev.map((p) => (p.id === partner.id ? { ...p, status: 'Rejected' as const } : p)))
...
const handleRedeemClick = (reward: Partner['reward'] & { partnerName: string }) => {
...
const allowedTypes = ['.csv', '.xlsx', '.xls', '.pdf', '.txt', '.docx', '.zip']
...
const path = `users/${user.uid}/uploads/${nowId}-${file.name}`
...
task.on(
  ...
  async () => {
    try {
      ...
      console.log("Uploading dataset object:", newDataset);

const sanitizedDataset = Object.fromEntries(
  Object.entries({
    ...newDataset,
    createdAt: serverTimestamp(),
      ...
      const docRef = await addDoc(datasetsCol, sanitizedDataset);

      console.log("✅ Dataset metadata saved to Firestore:", sanitizedDataset);

      // update local state (snapshot also updates)
      setUploadedDatasets((prev) => [{ ...newDataset, docId: docRef.id }, ...prev])
      ...
      setUploadError(e?.message || 'Failed to save dataset metadata.')
      toast.error(`Failed to save ${file.name}`, {
        description: e?.message || 'There was an error saving your file metadata.',
      })
...
return (Math.round((bytes / Math.pow(k, i)) * 100) / 100 + ' ' + sizes[i]) as string
}

// Use Storage URL if present; fallback to synthetic
const handleDownloadDataset = async (dataset: UploadedDataset) => {
try {
let url = dataset.downloadURL

  if (!url && dataset.storagePath) {
    const ref = storageRef(storage, dataset.storagePath)
  ...
  // Fallback to synthetic content (legacy behavior)
  let data = ''
...
// Delete both Firestore doc and Storage object
const handleDeleteDataset = async (datasetId: number) => {
const ds = uploadedDatasets.find((d) => d.id === datasetId)
...
try {
  if (user && ds?.docId) {
    await deleteDoc(doc(db, 'users', user.uid, 'datasets', ds.docId))
  }

  if (ds?.storagePath) {
    await deleteObject(storageRef(storage, ds.storagePath))
  }

  toast.success('Dataset deleted successfully')
} catch (err: any) {
  // Revert optimistic update on failure
  setUploadedDatasets((prev) => (ds ? [ds, ...prev] : prev))
...
const filteredUploadedDatasets = uploadedDatasets.filter((d) => {
...
Accept partner requests to unlock exclusive rewards. Your data, your choice, your benefits.
    ...
    {/* Main Tabs */}
    <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'upload' | 'partners' | 'rewards')} className="mb-8">
      <TabsList className="grid w-full max-w-2xl grid-cols-3 mb-6">
        ...
        <TabsTrigger value="rewards" className="text-base">
          ...
          Rewards
          {newlyUnlockedRewardId && <Sparkles className="w-4 h-4 ml-2 text-yellow-500 animate-pulse" />}
        </TabsTrigger>
      </TabsList>
            ...
            <Card className="border-dashed rounded-2xl">
              <CardContent className="py-12 text-center text-gray-500">
                ...
                <p className="text-sm mt-2">
                  {activeCategories.size === 0 ? 'Enable data categories to view eligible partners' : 'All partners have been reviewed'}
                </p>
              </CardContent>
            </Card>
          ) : (
                    ...
                    <AnimatePresence>
                      {filteredPartners
                        ...
                        .map((partner, index) => {
                          ...
                          const RewardIcon = getRewardIcon(partner.reward.icon, partner.reward.type)
                              ...
                              <Card className="hover:shadow-lg transition-all rounded-2xl h-full flex flex-col border border-gray-300 bg-white">
                                <CardHeader>
                                  ...
                                  <CardDescription className="space-y-3">
                                    {/* Data Types Required */}
                                    <div>
                                      <div className="text-xs uppercase tracking-wide mb-2">Data Types Requested</div>
                                      <div className="flex flex-wrap gap-1">
                                        {partner.dataTypesRequired.map((dataType) => {
                                          ...
                                          const hasDatasets = uploadedDatasets.some(
                                            (d) => d.category === dataType && d.status === 'success'
                                          )
                                          ...
                                          return (
                                            ...
                                            >
                                              ...
                                              {dataType}
                                              {isToggled && hasDatasets && <CheckCircle className="w-3 h-3 ml-1" />}
                                            </Badge>
                                          )
                                        })}
                                      </div>
          ...
          <Card className="border-dashed rounded-2xl">
            <CardContent className="py-16 text-center">
              <div className="max-w-md mx-auto">
                ...
                <p className="text-muted-foreground mb-6">Accept partner requests to unlock exclusive offers</p>
                <Button onClick={() => setActiveTab('partners')} className="bg-[#2563EB] hover:bg-[#2563EB]/90 rounded-xl">
                      ...
                      {filteredRewards
                        ...
                        .map((reward, index) => {
                          ...
                          return (
                            ...
                            >
                              <Card
                                className={`hover:shadow-lg transition-all rounded-2xl h-full flex flex-col bg-white border-[#0E1E3D]/10 ${
                              ...
                              >
                                ...
                                <CardContent className="space-y-4 flex-1 flex flex-col">
                                  ...
                                  >
                                    {reward.verificationStatus === 'pending' ? 'Awaiting Verification' : 'Redeem Now'}
                                  </Button>
                                </CardContent>
                              </Card>
                            </motion.div>
                      ...
                      {filteredRewards
                        ...
                        .map((reward, index) => {
                          ...
                          return (
                            ...
                            >
                              <Card
                                className={`hover:shadow-lg transition-all rounded-2xl h-full flex flex-col ${
      ...
      <TabsContent value="upload" className="space-y-6">
        ...
        <Alert className="border-blue-200 bg-blue-50/50">
          ...
          <AlertDescription className="text-blue-900">
            <strong>Supported formats:</strong> CSV, Excel (.xlsx, .xls), PDF, TXT, DOCX, ZIP. Maximum file size: 50MB.
        ...
        <Card className="rounded-2xl">
          <CardHeader>
            ...
            <CardDescription>Choose the category that best matches your data</CardDescription>
          </CardHeader>
          <CardContent>
                ...
                return (
                  <motion.div
                    ...
                    className={`flex items-start space-x-3 p-4 border-2 rounded-xl cursor-pointer transition-all ${
                      isSelected ? 'border-[#2563EB] bg-blue-50' : 'border-gray-200 hover:border-gray-300'
                    }`}
                  >
        ...
        <Card className="rounded-2xl">
          <CardHeader>
            ...
            <CardDescription>
              {selectedUploadCategory ? 'Drag and drop your file here or click to browse' : 'Please select a category first'}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div
            ...
            >
              <div className="flex flex-col items-center space-y-4">
                <div className={`w-16 h-16 rounded-full flex items-center justify-center ${selectedUploadCategory ? 'bg-primary/10' : 'bg-muted'}`}>
                  <Upload className={`w-8 h-8 ${selectedUploadCategory ? 'text-primary' : 'text-muted-foreground'}`} />
                </div>
                <div className="space-y-2">
                  <h3>
                    {!selectedUploadCategory ? 'Select a category to upload' : isDragging ? 'Drop your file here' : 'Upload your travel data'}
                  </h3>
                  <p className="text-sm text-muted-foreground">
                    {selectedUploadCategory
                      ? 'Upload anonymised data files (.csv, .xlsx, .pdf, .txt, .docx, .zip)'
                ...
                <input
                  ...
                  id="file-upload"
                  className="hidden"
                  accept=".csv,.xlsx,.xls,.pdf,.txt,.docx,.zip"
                  onChange={handleFileSelect}
                  disabled={!selectedUploadCategory || !user}
                />
                <label htmlFor="file-upload">
                  <Button asChild className="cursor-pointer rounded-xl" disabled={!selectedUploadCategory || !user}>
            ...
            {!user && (
              <Alert className="mt-4 border-yellow-200 bg-yellow-50/50">
                <Info className="h-4 w-4 text-yellow-600" />
                <AlertDescription className="text-yellow-900">Please sign in to upload and persist your datasets.</AlertDescription>
              </Alert>
            )}
            ...
            {uploadError && (
              <Alert className="mt-4 border-red-200 bg-red-50/50">
                ...
                <AlertDescription className="text-red-900">{uploadError}</AlertDescription>
              </Alert>
            )}
        ...
        <Card className="rounded-2xl">
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>View and manage your uploaded data files</CardDescription>
              </div>
          ...
          <CardContent>
            {isLoadingDatasets ? (
              <div className="text-center py-12 text-muted-foreground">Loading your datasets…</div>
                            ...
                            <TableCell>
                              <div className="flex items-center space-x-1">
                                <TooltipProvider>
                                  <Tooltip>
                                    <TooltipTrigger asChild>
                                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleDownloadDataset(dataset)}>
                ...
                {filteredUploadedDatasets.length > UPLOAD_ITEMS_PER_PAGE && (
                  <div className="flex items-center justify-between mt-4">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <div className="flex items-center space-x-1">
                        <span className="text-sm text-muted-foreground">Page {uploadCurrentPage} of {uploadTotalPages}</span>
                      </div>
                      <Button
                        variant="outline"
                        size="sm"
                ...
                {selectedPartner.dataTypesRequired.map((dataType) => {
                  ...
                  return (
                    <Badge
                      key={dataType}
                      variant="outline"
                      className={`${isMet ? 'bg-green-100 border-green-300 text-green-700' : 'bg-gray-100 border-gray-300 text-gray-700'}`}
                    >
...
<div className="space-y-3">
  ...
  {selectedPartner.dataTypesRequired.map((category) => {
    const datasetsInCategory = uploadedDatasets.filter(
      (d) => d.category === category && d.status === "success"
    );
    const allSelected = datasetsInCategory.every((d) =>
      selectedDatasets.includes(d.id)
    );
    const partiallySelected =
      datasetsInCategory.some((d) => selectedDatasets.includes(d.id)) &&
      !allSelected;

    if (datasetsInCategory.length === 0) {
      return (
        <div
          key={category}
        ...
        >
          ...
          <p className="text-sm text-muted-foreground mb-2">
            No datasets available for <strong>{category}</strong>.
          </p>
          <Button
            variant="outline"
            size="sm"
              ...
              onCheckedChange={(checked) => {
                if (checked) {
                  setSelectedDatasets((prev) => [
                    ...new Set([...prev, ...datasetsInCategory.map((d) => d.id)]),
                  ]);
                } else {
                  setSelectedDatasets((prev) =>
                    prev.filter(
                      (id) =>
                        !datasetsInCategory.map((d) => d.id).includes(id)
                    )
                  );
                }
              }}
  ...
  <span
    className={`px-2 py-0.5 rounded-md text-sm font-medium ${
      CATEGORY_STYLES[category]?.color || "bg-gray-100 text-gray-600"
    }`}
  >
    {category}
  </span>
            ...
            <span className="text-sm text-gray-500">
              ({datasetsInCategory.length} file
              {datasetsInCategory.length !== 1 ? "s" : ""})
            </span>
          </div>
        </div>

        <div className="divide-y">
          {datasetsInCategory.map((dataset) => {
            ...
            return (
              <div
                ...
                className={`flex items-center gap-3 p-3 cursor-pointer hover:bg-[#EAF1FF] transition-colors ${
        ...
        <div className="space-y-4">
          ...
          <div className="border rounded-xl p-4 space-y-3">
            <div className="flex items-start justify-between">
              ...
              <Badge className="bg-green-600 text-white shrink-0">{selectedReward.value}</Badge>
            </div>
